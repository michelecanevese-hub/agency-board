...
import { Box, Typography, Avatar, Button, Chip, Paper } from '@mui/material'
import AddIcon from '@mui/icons-material/Add'
import TaskCard from './TaskCard'
import { getInitials, stringToColor } from '../lib/constants'

export default function EmployeeColumn({ employee, tasks, commentCounts, onAddTask, onOpenTask, onMarkDone, hideHeader }) {
  const { setNodeRef, isOver } = useDroppable({ id: employee.id })
  const avatarColor = stringToColor(employee.name)

  return (
    <Box sx={{ width: hideHeader ? '100%' : 260, flexShrink: 0, display: 'flex', flexDirection: 'column', height: '100%' }}>
      {!hideHeader && (
        <Paper elevation={0} sx={{
          p: 1.5, mb: 1, borderRadius: 2,
          background: t => t.palette.mode === 'dark' ? 'rgba(255,255,255,0.04)' : 'rgba(0,0,0,0.03)',
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mt: 1 }}>
            <Chip label={`${tasks.length} task`} size="small"
      ...
      <Box ref={setNodeRef} sx={{
        flex: 1, minHeight: 120, borderRadius: 2,
        border: '2px dashed',
        borderColor: isOver ? 'primary.main' : 'transparent',
        bgcolor: isOver ? 'primary.main' + '11' : 'transparent',
        transition: 'all 0.15s', p: 0.5, overflowY: 'auto',
        ...
        {tasks.length === 0 && (
          <Box sx={{
            textAlign: 'center', py: 4, border: '1.5px dashed',
            borderColor: isOver ? 'primary.main' : 'divider', borderRadius: 2,
            bgcolor: isOver ? 'primary.main' + '08' : 'transparent', transition: 'all 0.15s',
          }}>
